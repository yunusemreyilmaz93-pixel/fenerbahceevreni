...
const formatArticleDate = (value?: string) => {
  ...
  return new Intl.DateTimeFormat('tr-TR', { day: 'numeric', month: 'short', year: 'numeric' }).format(parsed);
};
...
const LatestAnalysis: React.FC<LatestAnalysisProps> = ({ onNavigate, articles = [] }) => {
  const publishedArticles = useMemo(
    () => articles.filter((article) => article.status === 'published' || article.status === 'active' || !article.status),
    [articles]
  );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
            ...
            {featuredArticle && (
              <motion.button
                ...
                className="lg:col-span-7 rounded-2xl bg-[#111625] border border-white/[0.08] hover:border-[#FFD21F]/30 p-8 flex flex-col justify-between transition-[border-color,transform,background-color,color] duration-200 cursor-pointer group shadow-lg hover:scale-[1.005]"
              >
                <div className="space-y-6 text-left">
                  {featuredArticle.coverImage && (
                    <div className="aspect-[16/9] overflow-hidden rounded-xl border border-white/[0.06]">
                      <img
                  ...
                  <div className="flex justify-between items-center pb-4 border-b border-white/[0.04]">
                    ...
                    <span className="text-[10px] font-bold text-slate-400 flex items-center gap-1.5 font-mono">
                      <Clock aria-hidden="true" className="w-3.5 h-3.5" /> {featuredArticle.readingTime || '6 dk okuma'}
                    </span>
                  </div>

                  {/* Title and Excerpt */}
                  <div className="space-y-4">
                    <h3 className="text-2xl md:text-3xl font-display font-black text-white leading-tight text-balance group-hover:text-[#FFD21F] transition-colors">
                ...
                <div className="pt-8 mt-8 border-t border-white/[0.05] flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    <div className="w-9 h-9 rounded-full bg-white/[0.03] border border-white/10 flex items-center justify-center text-slate-300">
                      <User aria-hidden="true" className="w-4 h-4 text-[#FFD21F]" />
                    </div>
                    <div>
                      <div className="text-xs font-black text-white uppercase italic">{featuredArticle.author || 'Analiz Ekibi'}</div>
                      <span className="text-[10px] text-slate-400 font-bold">{formatArticleDate(featuredArticle.date)}</span>
                    </div>
                  </div>
                  
                  <span className="flex items-center gap-2 text-xs font-black text-[#FFD21F] uppercase tracking-widest bg-[#FFD21F]/10 border border-[#FFD21F]/20 p-2.5 px-4 rounded-xl group-hover:bg-[#FFD21F] group-hover:text-[#0B0F19] transition-colors duration-200">
              ...
              {secondaryArticles.length > 0 ? (
                secondaryArticles.map((art, idx) => (
                  <motion.button
                    ...
                    className="p-5 rounded-2xl bg-[#111625] border border-white/[0.06] hover:border-[#FFD21F]/30 hover:scale-[1.01] flex flex-col justify-between transition-[border-color,transform,background-color,color] duration-200 cursor-pointer group text-left flex-1"
                  >
                    <div className="space-y-2.5">
                      <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-wider text-slate-400 font-mono">
                        ...
                        <span className="flex items-center gap-1">
                          <Clock aria-hidden="true" className="w-3" /> {art.readingTime || '4 dk okuma'}
                        </span>
                      </div>
                      <h4 className="text-lg font-bold text-white leading-snug group-hover:text-[#FFD21F] transition-colors line-clamp-2 text-balance">
                        {art.title}
                      </h4>
                    ...
                    <div className="flex justify-between items-center mt-4 pt-4 border-t border-white/[0.05]">
                      <span className="text-[10px] font-bold text-slate-400 uppercase font-mono">
                        {art.author || 'Fenerbahçe Evreni'} • {formatArticleDate(art.date)}
                      </span>
